...
window.resizable(0,0) #to prevent resizing
...
input_frame = Frame(window, width=312, height=50, bd=0)#bd is border
input_frame.pack(side=TOP)

input_field = Entry(input_frame, font=('arial',18,'bold'), textvariable=input_text, width=50, bg='#eee', justify=RIGHT)
input_field.grid(row=0, column=0)
input_field.pack(ipady=10) #ipady or ipadx is internal padding from both sides to increase the height of input_field

btns_frame = Frame(window, width=312, height=272.5, bg='grey')
btns_frame.pack()
#fg for color while the cursor is on the widget #hand2 to change the cursor for a hand shape
#columnspan تتمدد على كم عمود
